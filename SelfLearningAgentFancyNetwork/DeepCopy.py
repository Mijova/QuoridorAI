        ...
        with torch.no_grad():
          ...
          for state, action, reward, next_state, is_terminal in batch:
            if is_terminal:
              ...
              state_action_q_values_target[id][action] = - GAMMA * torch.max(next_state_action_q_values[id]+fancy_mask(next_state).to(device))
            id = id + 1
...
def main():
    ...
    for episode in range(1, EPISODES+1):
        ...
        if (episode % 1000 == 0):
            torch.save(agent, f"rSelfLearnedER{BOARD_SIZE}x{BOARD_SIZE}-{episode}")
    end = time.time()
