...
class DQNAgentReplay():
    ...
    def backward(self,game):
        ...
        for state, action, reward, next_state, is_terminal in batch:
            ...
            if (first_terminal == len(batch) and is_terminal): first_terminal = id
            id = id + 1
        state_action_q_values = self.current.forward(batch_nn_input)
        non_terminal_batch = (batch_nn_input[0][:first_terminal], batch_nn_input[1][:first_terminal])
        with torch.no_grad():
          state_action_q_values_target = state_action_q_values.clone().detach()
          next_state_action_q_values = self.target.forward(non_terminal_batch)
          id = 0
          for state, action, reward, next_state, is_terminal in batch:
            if is_terminal:
              state_action_q_values_target[id][action] = reward
            else:
              if (next_state.current_player() == 1):
                next_state_action_q_values[id] = next_state_action_q_values[id][ROTATION_MASK]
              state_action_q_values_target[id][action] = - GAMMA * torch.max(next_state_action_q_values+fancy_mask(next_state).to(device))
            id = id + 1
...
def main():
    ...
    for episode in range(1, EPISODES+1):
        ...
        while not state.is_terminal():
            ...
            agent.remember(old_state, rotated_action, rewards[pl], state.clone(), state.is_terminal())
        ...
        if (episode % 5000 == 0):
            torch.save(agent, f"batchSelfLearnedER{BOARD_SIZE}x{BOARD_SIZE}-{episode}")
    end = time.time()
